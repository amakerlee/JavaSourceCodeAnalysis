...
* 计算在新数组中的位置：JDK 1.7 中对每一个 key 采用原来的计算方法（hashCode & (length - 1）进行计算，JDK 1.8 中根据新增参与运算位进行计算（原位置 / 原位置 + 扩容前数组容量）。
* 转移数据：JDK 1.7 中使用头插法，将原位置数据移到后一位，再插入数据到该位置，会出现逆序，环形链表死循环等问题；JDK 1.8 中直接插入链表尾部 / 红黑树，不会出现逆序，环形链表死循环等问题。
* 插入时间：JDK 1.7 在扩容后插入，单独计算每一个节点位置；JDK 1.8 在扩容前插入，转移数据前统一计算位置。

### hashCode() 和 equals()

key 如果为 Object 类型，则需要实现 hashCode 方法和 equals 方法。

hashCode 方法用来计算需存储数据的存储位置，如果实现不恰当会导致严重的哈希碰撞问题。
...
计算 key 落在数组中哪一个桶里时，用 key 的 hash 值和 table 的长度 length 作取余运算，而仅当 length 为 2 的幂时，才有 key.hashCode % table.length 等于 key.hashCode & (table.length - 1)，原因如下：任何 2 的幂除最高位是 1 之外，其它位都为 0，减 1 之后，最高位变成 0，比最高位低的所有位次变成 1，此时和 key.hashCode 作 & 运算，相当于保留 hashCode 的低位，即相当于对 length 取余。由于位运算的效率要高于 % 运算，所以选择了位运算而不是 % 运算。

PS：如果事先选定使用 key.hashCode & (table.length - 1) 运算，那么设定 length 为 2 的幂也是合理的，因为只有这种情况下低位才全都是 1，位置完全取决于 hashCode 的低位掩码，而不会出现有的桶不可能命中的情况。

**数组扩容时，仅仅关注 “特殊位” 就可以重新定位元素**

数组扩容时，新数组的长度变成原数组的 2 倍，例如 1000 -> 10000，length - 1 则相应为 111 -> 1111。对于每一个 key 重新计算其位置时，原本被舍弃的高位的最后一位，将会参与计算。如果这个位置是 0，那么计算结果不变，如果是 1，计算结果变成了原槽的索引 + 原数组长度。不难看出只要检查这一个“特殊位”，就能确定键值对在新数组中的位置。

综上所述，容量设置为 2 的幂其实是为了提高 HashMap 的性能，特别是计算 key 所在位置的时候，提高了将会频繁使用的 key.hashCode & (table.length - 1) 运算的效率。
...
```
扩容机制：当哈希表的大小大于等于扩容阈值（容量乘负载因子）时，对哈希表扩容。
...
在JDK 1.7 中，重新调整 HashMap 大小的时候，可能存在条件竞争，因为如果两个线程都发现 HashMap 需要重新调整大小了，它们会同时试着调整大小。在调整大小的过程中，存储在链表中的元素的次序会反过来，因为移动到新的 bucket 位置的时候，HashMap 并不会将元素放在链表的尾部，而是放在头部，这是为了避免尾部遍历(tail traversing)，原数组[j]位置上的桶移到了新数组[j+原数组长度]。如果条件竞争发生了，那么就死循环了（形成环形链表）。
由于 JDK 1.8 转移数据操作是按旧链表的正序遍历链表、在新链表的尾部依次插入，所以不会出现链表 逆序、倒置的情况，故不会出现环形链表的情况。但是在 put 操作中，两个线程同时进行插入操作，一个线程 A 获取插入位置（链表尾部）之后等待时间片，另一个线程 B 完成插入操作，等到 A 进行插入操作时，其保存的位置已经不是链表尾部，此时插入将会覆盖线程 B 的数据。

可通过 Collections 类的静态方法 synchronizedMap 获得线程安全的 HashMap（效率低），或者使用 ConcurrentHashMap。
